...
// for timer
obj.start_time=document.getElementById("timer_start_time") as HTMLInputElement;
obj.deadline_time=document.getElementById("timer_time") as HTMLInputElement;


obj.date_field=document.getElementById("date_field") as HTMLInputElement;
obj.text_field=document.getElementById("text_field") as HTMLInputElement;
...
function func_main_1():void{
    if(obj.date_field.value!="" && obj.text_field.value!="" &&obj.start_time.value!="" &&obj.deadline_time.value!="")
    {
        ...
        let temp=new display_date_time();
        let d1=temp.display_date_time(x);
        ...
        //d1+=" "+start_time.value;
        let deadline_timer=new Date(obj.deadline_time.value).getTime();
        //console.log(deadline_timer);

        if(goal_start_timer<=current_timer && current_timer<deadline_timer) //present
        {
            obj.present_array.push(new container(obj.div_left,x,obj.text_field.value,"present",deadline_timer,deadline_timer,obj));
        ...
        {
            obj.future_array.push(new container(obj.div_middle,x,obj.text_field.value,"future",goal_start_timer,deadline_timer,obj));
